"""weather_tail_buy — buy YES at $0.01-0.05 on weather-category buckets.
...
@dataclass
class WeatherTailBuyParams:
    min_price: Decimal = Decimal("0.01")
    max_price: Decimal = Decimal("0.05")
    target_notional_usd: Decimal = Decimal("25")
    price_buffer_bps: int = 100
    recent_adverse_threshold: Decimal = Decimal("0.005")
    recent_window_secs: int = 600
    max_concurrent_positions: int = 100
    category_keywords: tuple[str, ...] = ("weather",)


class WeatherTailBuy:
    name: str = "weather_tail_buy"
    edge_class: str = "tail"

    def __init__(self, **params: Any) -> None:
        kw = {k: v for k, v in params.items() if k in WeatherTailBuyParams.__annotations__}
        if "category_keywords" in kw and isinstance(kw["category_keywords"], list):
            kw["category_keywords"] = tuple(kw["category_keywords"])
        self.params = WeatherTailBuyParams(**kw)
    ...
    async def on_event(self, event: MarketEvent, state: dict[str, Any]) -> list[Signal]:
        ...
        if event.event_type == EventType.TRADE_PRINT and event.asset_id is not None:
            ...
            cutoff = event.ts - timedelta(seconds=900)
        ...
        # Adverse-print filter: print at-or-below threshold = "this is genuinely
        # cheap because it's worthless" — skip
        recent = state.get("_recent_prints", {}).get(asset_id, [])
        cutoff = event.ts - timedelta(seconds=self.params.recent_window_secs)
        threshold = self.params.recent_adverse_threshold
        if any(p <= threshold and t >= cutoff for (t, p) in recent):
            return []

        active: set[tuple[str, str]] = state.setdefault("_active_buys", set())
        ...
        return [Signal(
            ...
            price=price,
            size=size,
            reason=f"weather_tail_buy: ask={ask:.4f} cat={cat}",
            ts_signal=event.ts,
            metadata={"ask_at_signal": str(ask), "category": cat},
        )]


def plugin() -> WeatherTailBuy:
    return WeatherTailBuy()
